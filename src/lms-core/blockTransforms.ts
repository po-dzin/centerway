...
export function changeNodeKind(content: RichTextNode[], index: number, kind: RichTextNode["kind"]): RichTextNode[] {
  ...
  return content.map((current, position) =>
    position !== index ? current : kind === "ul" || kind === "ol" ? { kind, items } : { kind, text: joinInline(items) },
  );
}
...
export function transformRichNode(
  ...
  const replacement: LessonBlock =
    kind === "quote"
      ? {
          id,
          type: kind,
          text: items.some((item) => inlineToPlainText(item).trim())
            ? joinInline(items.filter((item) => inlineToPlainText(item).trim()))
            : "[ЗАПОВНИ: цитата]",
        }
      : kind === "code"
        ...
        : {
            ...
            items: items.map((text) => ({
              id: ids(),
              text: inlineToPlainText(text).trim() ? text : "[ЗАПОВНИ: пункт]",
            })),
          };
  return [
