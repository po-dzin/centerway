...
export function courseReadiness(course: Course): CourseReadiness {
  ...
  if ((course.visibility ?? "hidden") !== "hidden") {
    /* THE ADDRESS IS THE ONE FIELD NOBODY LOOKS AT.
     *
     * The builder names a new draft «Новий курс N» and derives the slug from
     * that title. The author then renames the course — the title is in front of
     * them — and the slug silently keeps the invented one. That is how a real
     * course reached the storefront titled «Soul Daily Ritual» at
     * `/programs/novyi-kurs-5`: listed, in the sitemap, no `noindex` on
     * `/programs/*` at all, and the slug already written into `course:<slug>`
     * on fourteen orders.
     *
     * Conditional on visibility for the same reason as the cover below: a
     * hidden draft is allowed to be unfinished, and blocking a default slug at
     * SAVE time would break the one-click create it is generated by. The moment
     * the course points at strangers the address stops being a placeholder and
     * becomes permanent — a slug can be changed while nothing depends on it and
     * not after, so the last honest moment to ask is this one. */
    if (isDefaultDraftSlug(course.slug)) add("lms_ready_default_slug", course.slug);
