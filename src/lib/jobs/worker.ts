...
function isCapiEventPayload(payload: unknown): payload is CapiEventPayload {
  ...
  return typeof p.event_name === "string" && typeof p.event_id === "string" && typeof p.event_time === "number";
}
  ...
  return {
    ...
    fbc_creation_time_seconds:
      ...
      parseIsoToUnixSeconds(
        typeof initiateCheckoutJobRes.data?.created_at === "string" ? initiateCheckoutJobRes.data.created_at : null,
      ) ??
      parseIsoToUnixSeconds(
        typeof checkoutStartedEventRes.data?.created_at === "string" ? checkoutStartedEventRes.data.created_at : null,
      ) ??
      payload.payment_event_time ??
      Math.floor(Date.now() / 1000),
    value: typeof orderTracking?.amount === "number" ? orderTracking.amount : payload.value,
    currency: normalizeTrackingString(orderTracking?.currency) ?? normalizeTrackingString(payload.currency) ?? "UAH",
    order_ref: orderRef,
    ...
    fbc: normalizeTrackingString(initiatePayload?.fbc) ?? normalizeTrackingString(checkoutPayload?.fbc),
    fbclid:
