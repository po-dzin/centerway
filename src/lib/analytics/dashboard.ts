/**
 * The admin dashboard's engine: every read and every aggregate behind
 * GET /api/admin/analytics, moved out of the route handler on 2026-09-10.
 *
 * It was 1,800 lines of SQL and arithmetic inside src/app/api/admin/analytics/
 * route.ts, which the Telegram reports could not share and the page could only
 * mirror by hand. The route is the HTTP skin now; this is the engine, and its
 * payload type is what the page renders against.
 */
...
export type CampaignBreakdownLevel = "adset" | "ad";
...
export function campaignBreakdownLevelFromQuery(searchParams: URLSearchParams): CampaignBreakdownLevel {
  ...
  for (const row of rows) {
    ...
    if (rawName) {
      ...
      if (key) {
        ...
        const value =
          valueCandidates.map((candidate) => Number(candidate)).find((n) => Number.isFinite(n) && n >= 0) ?? 0;
        totals[key] += value;
      }
    }
...
async function fetchPixelTotals(range: DateRange): Promise<PixelTotalsResult> {
  const pixelId = process.env.META_PIXEL_ID || process.env.META_AD_PIXEL_ID || process.env.META_PIXEL;
  const token = process.env.META_ADS_ACCESS_TOKEN || process.env.META_ACCESS_TOKEN || process.env.META_CAPI_TOKEN;
...
export async function computeAnalyticsPayload(range: DateRange, campaignLevel: CampaignBreakdownLevel) {
  ...
  const { data: revenueOrders, error: revErr } = await db
    .from("orders")
    .select(
      "order_ref, campaign, page_url, product_code, status, amount, created_at, fbclid, fbp, client_ip, client_ua",
    )
    .gte("created_at", range.fromTs)
    .lt("created_at", range.toExclusiveTs)
    .limit(50000);
  ...
  for (const row of revenueOrderRows) {
    const createdAt = typeof row.created_at === "string" ? row.created_at : null;
    const dayKey = createdAt ? getIsoDateInTimeZone(new Date(createdAt), ANALYTICS_TZ) : null;
  ...
  for (const rawRow of pixelRows ?? []) {
    ...
    const shouldReplace =
      !current || (nextSyncedAt && currentSyncedAt ? nextSyncedAt > currentSyncedAt : Boolean(nextSyncedAt));
  ...
  const pixelDailyExpectedDays =
    Number.isFinite(periodFromTs) && Number.isFinite(periodToTs) && periodToTs >= periodFromTs
      ...
      : 0;
  const pixelDailyCoveragePercent =
    pixelDailyExpectedDays > 0 ? Number(((pixelDailyDaysPresent * 100) / pixelDailyExpectedDays).toFixed(2)) : 0;
  ...
  if (marketingErr) {
    ...
    const manualUpdatedAt = typeof marketingData.updated_at === "string" ? marketingData.updated_at : null;
    ...
    marketingInputs = {
      ...
      period_label:
        manualOverlapsSelectedPeriod && typeof marketingData.period_label === "string"
  ...
  if (hasMetaAggregate) {
    marketingInputs = {
      ...
      period_label: metaAggregate.latest_day
        ? `meta synced up to ${metaAggregate.latest_day}`
  ...
  const alignedFromTs = firstScrollDepthAt && firstScrollDepthAt > range.fromTs ? firstScrollDepthAt : range.fromTs;
  ...
  const engagement: EngagementStats = {
    ...
    scroll50_to_checkout_percent: Number(safeDivide(alignedInitiateCheckoutTotal * 100, scrollDepth50Total).toFixed(2)),
  ...
  const pixelDailyLowCoverageWhenPixelUnavailable =
    pixelStatsUnavailable && hasPixelDailyViewContent && pixelDailyCoveragePercent < 99;
  ...
  const resolvedViewContent = usePixelDailyForViewContent
    ...
    : localViewContent > 0
      ...
      : fallbackViewContent;
  const viewContentSource = preferReferencePixelViewContent
  ...
  const { data: qualityRow, error: qualityErr } = await db
    ...
    .select(
      "snapshot_date, paid_missing_fbclid, paid_missing_fbp, paid_missing_page_url, paid_missing_client_ip, paid_missing_client_ua",
    )
  ...
  if (paidOrderRefs.length > 0) {
    ...
    try {
      ...
      console.warn("Analytics purchase transport warning:", error instanceof Error ? error.message : String(error));
    }
  }
  ...
  const quality_series = Array.from(qualitySeriesMap.values()).sort((a, b) => a.date.localeCompare(b.date));
  const quality_gaps: QualityGaps | null =
    quality_series.length > 0
  ...
  return {
    ...
    funnel_chain: {
      ...
      purchase_to_access_percent: toFixed2(safeDivide(businessTotals.access_granted * 100, businessTotals.purchase)),
    },
...
export function getCachedAnalyticsPayload(range: DateRange, campaignLevel: CampaignBreakdownLevel) {
...
/** What GET /api/admin/analytics returns — the page renders against this, not a copy. */
export type AnalyticsPayload = Awaited<ReturnType<typeof computeAnalyticsPayload>>;
