...
import { syncLandingCommerce } from "./landingPrices";
...
export async function prepareLandingHtml(
  ...
  if (options.pageKind === "entry") {
    ...
    /* The printed price follows the charged one, and a CTA is only left able
       to charge when there is a price to charge. Opt-in per element via
       `data-cw-price`, and a product with nothing to read keeps the figure
       typed into the HTML — see `priceSync`. */
    html = await syncLandingCommerce(html);
