  ...
  db.tables = {
    ...
    lms_courses: [
      ...
      },
      {
        ...
        locale: "uk",
        brand: "centerway",
        author_id: "auth-coach",
        author_profile_id: null,
        updated_at: daysAgo(1),
      },
    ],
    lms_authors: [{ id: "profile-1", slug: "coach", name: "Coach", auth_user_id: "auth-coach" }],
    lms_lessons: [
...
describe("listPeople — the enrollment side", () => {
  ...
  it("keeps a learner visible when their course row is missing", async () => {
    ...
    const orphan = items.flatMap((account) => account.courses).find((row) => row.enrollmentId === "enr-orphan");
  ...
  it("matches a person when any one of their courses is in the filtered status", async () => {
    ...
    expect(idle.items.map((row) => row.email).sort()).toEqual(["fresh@example.com", "learner@example.com"]);
  });
...
describe("grantCourse", () => {
  ...
  it("refuses an unknown course", async () => {
    await expect(grantCourse({ email: "fresh@example.com", courseSlug: "nope", actorId: ADMIN })).rejects.toMatchObject(
      { message: "course_not_found", status: 404 },
    );
  });
...
describe("listPeople — the account side", () => {
  ...
  it("narrows to the elevated accounts, and only those", async () => {
    ...
    expect(items.map((row) => row.email)).toEqual(expect.arrayContaining(["admin@example.com", "coach@example.com"]));
...
describe("setRole", () => {
  ...
  it("refuses an account that has never signed in", async () => {
    await expect(setRole({ email: "ghost@example.com", role: "admin", actorId: ADMIN })).rejects.toMatchObject({
...
describe("course moderation and admin deletion", () => {
  ...
  it("journals the exact document it publishes, so an approval can be proved after the fact", async () => {
    ...
    const revision = {
      ...
      id: "course-reset",
      slug: "reset-day",
      title: "Точний перевірений документ",
      status: "draft",
    };
    const row = db.rows("lms_courses").find((item) => item.id === "course-reset")!;
    Object.assign(row, {
  ...
  it("requires the exact slug, then lets an admin delete a course with learners", async () => {
    await expect(
      deleteAdminCourse({ courseId: "course-reset", confirmSlug: "wrong", actorId: ADMIN }),
...
describe("setCourseAuthorProfile", () => {
  ...
  it("refuses an unknown profile without changing the course", async () => {
    await expect(
      setCourseAuthorProfile({ courseId: "course-reset", authorProfileId: "missing", actorId: ADMIN }),
  ...
  it("refuses a missing course before reading or changing a profile", async () => {
    await expect(
      setCourseAuthorProfile({ courseId: "missing", authorProfileId: "profile-1", actorId: ADMIN }),
    ).rejects.toMatchObject({ message: "course_not_found", status: 404 });
    expect(db.rows("lms_authors")[0]).toMatchObject({ id: "profile-1", name: "Coach" });
    expect(auditRows()).toHaveLength(0);
  });
