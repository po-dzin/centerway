...
export async function listCatalog(): Promise<CatalogRow[]> {
  ...
  const emailByAuthor = new Map(
    (authorRows ?? []).map((row) => [row.auth_user_id as string, row.email as string | null]),
  );
  ...
  if (pending.length > 0) {
    ...
    const submitterIds = [
      ...new Set((submissionRows ?? []).map((row) => row.created_by as string | null).filter(Boolean)),
    ] as string[];
  ...
  return courses.map((row) => {
    ...
    const reviewStatus = (row.review_status as string | null) ?? (row.status === "published" ? "approved" : "draft");
