...
export async function listPeople(input: ListPeopleInput): Promise<{
  ...
  const roleUpdatedById = new Map(
    (roles ?? []).map((row) => [row.user_id as string, (row.updated_at as string | null) ?? null]),
  );
...
async function customersByAccount(
  ...
  const [linked, byEmail] = await Promise.all([
    ids.length > 0 ? db.from("customers").select("id, auth_user_id, email").in("auth_user_id", ids) : { data: [] },
