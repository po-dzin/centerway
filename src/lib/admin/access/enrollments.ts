...
export async function setEnrollmentDeadline(input: {
  ...
  const { error } = await db.from("lms_enrollments").update({ expires_at: input.expiresAt }).eq("id", enrollment.id);
  if (error) throw new AccessError(error.message, 500);
...
async function enrollmentContext(db: ReturnType<typeof adminClient>, enrollmentId: string) {
  ...
  return {
    enrollment,
    courseSlug: (course?.slug as string | null) ?? null,
 ...
 */
export async function reactivateCourse(input: { enrollmentId: string; actorId: string; expiresAt?: string | null }) {
  const db = adminClient();
  const { enrollment, courseSlug, email } = await enrollmentContext(db, input.enrollmentId);
  ...
  await writeAudit(db, {
    ...
    metadata: {
      ...
      expires_at_before: (enrollment.expires_at as string | null) ?? null,
      expires_at_after:
        input.expiresAt === undefined ? ((enrollment.expires_at as string | null) ?? null) : input.expiresAt,
    },
  });
