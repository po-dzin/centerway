...
export async function listCourses(): Promise<CourseRow[]> {
  ...
  const { data: courses, error } = await db.from("lms_courses").select("*").order("updated_at", { ascending: false });
  if (error) throw new AccessError(error.message, 500);

  const authorIds = [
    ...new Set((courses ?? []).map((row) => row.author_id as string | null).filter((id): id is string => Boolean(id))),
  ];
  ...
  return (courses ?? []).map((row) => {
    ...
    return {
      ...
      reviewStatus: row.pending_content
        ...
        : ((row.review_status as CourseRow["reviewStatus"] | undefined) ??
...
export async function deleteAdminCourse(input: { courseId: string; confirmSlug: string; actorId: string }) {
  ...
  const { error: auditError } = await db.from("audit_log").insert({
    ...
    action: "course.delete_requested",
  ...
  await writeAudit(db, {
    ...
    metadata: { slug: course.slug, learners: count ?? 0 },
  });
...
export async function moderateCourse(input: {
  ...
  if (input.action === "approve") {
    ...
    values = hasPendingRevision
      ...
      : {
          ...
          review_note: input.note?.trim() || "Потрібні зміни",
          approved_at: null,
          approved_by: null,
        };
  } else {
    if (!input.visibility || !["hidden", "unlisted", "listed"].includes(input.visibility))
  ...
  await writeAudit(db, {
    ...
    entityType: "lms_course",
    entityId: input.courseId,
    metadata: { slug: course.slug, ...values },
  });
...
export async function setCourseAuthorProfile(input: {
  courseId: string;
  ...
  const { error } = await db
    .from("lms_courses")
    .update({ author_profile_id: input.authorProfileId })
    .eq("id", input.courseId);
  if (error) throw new AccessError(error.message, 500);
  await writeAudit(db, {
    actorId: input.actorId,
