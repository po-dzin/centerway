        ...
        for (const row of this.payload) {
          ...
          if (clash) {
            return {
              ...
              error: {
                code: "23505",
                message: `duplicate key value violates unique constraint on ${this.table}(${columns.join(", ")})`,
              },
            };
          }
        }
  ...
  rpc = async (
    ...
    if (name === "create_lms_course_revision_once") {
      ...
      if (last && last.content_hash === args.p_content_hash) {
        return {
          data: [{ id: last.id, revision_number: last.revision_number, created_at: last.created_at, created: false }],
          error: null,
        };
      }
