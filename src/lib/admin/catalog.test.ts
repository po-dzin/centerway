...
describe("saleBlockersOf", () => {
  ...
  it("does not nag about approval before the author has published at all", () => {
    expect(saleBlockersOf({ status: "draft", reviewStatus: "draft", visibility: "hidden", offer: null })).toEqual([
      "not_published",
      "hidden",
      "no_offer",
    ]);
...
describe("saveOffer", () => {
  ...
  it("refuses an offer that says nothing about time", async () => {
    await expect(saveOffer({ courseId: "course-reset", actorId: ADMIN, amount: 990 })).rejects.toMatchObject({
      message: "access_rule_required",
    });
  });
...
describe("setOfferActive", () => {
  ...
  it("refuses a course that was never priced", async () => {
    ...
    await expect(setOfferActive({ courseId: "course-reset", active: false, actorId: ADMIN })).rejects.toMatchObject({
