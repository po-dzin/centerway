...
describe("authentication", () => {
  it("refuses every endpoint without an admin session, without touching the data", async () => {
    ...
    const responses = await Promise.all([
      ...
      learners.PATCH(
        send("http://x/api/admin/access/learners", "PATCH", { enrollmentId: "e1", expiresAt: "2026-09-30" }),
      ),
      learners.GET(get("http://x/api/admin/access/learners")),
...
describe("learners", () => {
  it("passes filters through and reports the caps it applied", async () => {
    await learners.GET(
      get("http://x/api/admin/access/learners?q=ann&course=reset-day&status=stalled&limit=10&offset=20"),
    );
  ...
  it("lets support grant and revoke course access", async () => {
    ...
    const granted = await learners.POST(
  ...
  it("treats a missing deadline key as 'not overridden', so the offer's own term applies", async () => {
    ...
    expect(access.provisionAccess).toHaveBeenCalledWith(expect.objectContaining({ expiresAt: undefined }));
  });
  ...
  it("sets and clears a deadline on one enrollment", async () => {
    ...
    const noTarget = await learners.PATCH(
      send("http://x/api/admin/access/learners", "PATCH", { expiresAt: "2026-09-30" }),
    );
  ...
  it("keeps an AccessError's own status instead of turning it into a 500", async () => {
    ...
    const res = await learners.POST(
      send("http://x/api/admin/access/learners", "POST", { email: "ghost@b.c", course: "reset-day" }),
    );
    ...
    expect(
      (await learners.POST(send("http://x/api/admin/access/learners", "POST", { email: "a@b.c", course: "nope" })))
        .status,
    ).toBe(404);
  });
...
describe("accounts", () => {
  ...
  it("pages the people list and tells the UI whether roles may be changed", async () => {
    ...
    expect(res.status).toBe(200);
    expect(access.listPeople).toHaveBeenCalledWith(expect.objectContaining({ q: "ann", limit: 10, offset: 20 }));
...
describe("courses", () => {
  ...
  it("refuses an authorship change from support", async () => {
    session.value = SUPPORT;
    const res = await courses.PATCH(
      send("http://x/api/admin/access/courses", "PATCH", { courseId: "c1", email: "a@b.c" }),
    );
    expect(res.status).toBe(403);
  ...
  it("reads a blank email as 'return it to the house', not as a missing field", async () => {
    ...
    const res = await courses.PATCH(
      send("http://x/api/admin/access/courses", "PATCH", { courseId: "c1", email: "   " }),
    );
    expect(res.status).toBe(200);
  ...
  it("changes only the public author-profile link through its explicit action", async () => {
    ...
    const res = await courses.PATCH(
      send("http://x/api/admin/access/courses", "PATCH", {
        courseId: "c1",
