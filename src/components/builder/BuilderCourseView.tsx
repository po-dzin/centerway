          ...
          <p className={styles.panelText}>
            {state.data.hasPendingRevision
              ...
              : state.data.review.enabled
                ? /* Схвалення ПУБЛІКУЄ (див. `moderateCourse`), і рядок каже
                     саме це: автора більше не просять повернутися сюди й
                     натиснути другу кнопку. Каталог — окреме рішення, і це
                     єдине, що лишилось по той бік. */
                  "Збережіть готову структуру й надішліть її на перевірку. Щойно її схвалять, курс стане доступним учням; видимість у каталозі окремо визначає адміністратор."
