...
import { ORDER_STATUSES, isOrderStatus } from "@/lib/payments/orderStatus";
...
export async function PATCH(req: NextRequest) {
  ...
  if (status === "paid" && session.role !== "admin") {
    ...
    return NextResponse.json({ error: "Confirming a payment requires an admin account" }, { status: 403 });
  }
  ...
  const { data, error } = await db.from("orders").update({ status }).eq("order_ref", order_ref).select().single();
