(function () {
  ...
  function setupCtas() {
    var ctas = document.querySelectorAll(".js-consult-cta");
  ...
  function setupVariantSwitch() {
    var toggle = document.getElementById("variant-switch");
    var altShell = document.getElementById("variant-alt-shell");
    var altFrame = document.getElementById("variant-alt-frame");
    var defaultNodes = document.querySelectorAll(".variant-default-node");
    var storageKey = "cw_consult_variant";

    if (!toggle || !altShell || !altFrame || !defaultNodes.length) return;

    function applyMode(isAlt) {
      defaultNodes.forEach(function (node) {
        node.classList.toggle("is-hidden-by-variant", isAlt);
      });

      altShell.hidden = !isAlt;

      if (isAlt && !altFrame.getAttribute("src")) {
        altFrame.setAttribute("src", "/consult-alt/index.html");
      }

      try {
        localStorage.setItem(storageKey, isAlt ? "alt" : "base");
      } catch (_) {}
    }
    ...
    var startWithAlt = savedMode === "alt";
    toggle.checked = startWithAlt;
    applyMode(startWithAlt);
