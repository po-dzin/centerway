...
const cases = [
  ...
  { name: "lms-core rejects a package import", file: "src/lms-core/__guard_probe.ts", expect: "zero dependencies",
    code: `import { z } from "zod";\nexport const a = z;\n` },
  { name: "lms-core rejects React", file: "src/lms-core/__guard_probe.ts", expect: "zero dependencies",
    code: `import * as React from "react";\nexport const a = React;\n` },
  { name: "lms-core rejects a node builtin", file: "src/lms-core/__guard_probe.ts", expect: "zero dependencies",
    code: `import fs from "node:fs";\nexport const a = fs;\n` },
  { name: "lms-core rejects the @/ alias", file: "src/lms-core/__guard_probe.ts", expect: "zero dependencies",
    code: `import { env } from "@/lib/env";\nexport const a = env;\n` },
  { name: "lms-core rejects a re-export from a package", file: "src/lms-core/__guard_probe.ts", expect: "zero dependencies",
    code: `export * from "@supabase/supabase-js";\n` },
  { name: "lms-core allows a relative sibling", file: "src/lms-core/__guard_probe.ts", expect: null,
  ...
  { name: "lms-core rejects window", file: "src/lms-core/__guard_probe.ts", expect: "outside a browser",
    code: `export function a() { return window.innerWidth; }\n` },
  { name: "lms-core rejects document", file: "src/lms-core/__guard_probe.ts", expect: "outside a browser",
    code: `export function a() { return document.title; }\n` },
  { name: "lms-core rejects localStorage", file: "src/lms-core/__guard_probe.ts", expect: "storage is the caller",
    code: `export function a() { return localStorage.getItem("k"); }\n` },
  { name: "lms-core rejects fetch", file: "src/lms-core/__guard_probe.ts", expect: "stay pure",
    code: `export async function a() { return fetch("/x"); }\n` },
  { name: "lms-core rejects process.env", file: "src/lms-core/__guard_probe.ts", expect: "configuration as arguments",
    code: `export function a() { return process.env.X; }\n` },
  { name: "lms-core rejects JSX", file: "src/lms-core/__guard_probe.tsx", expect: "renderer leaked in",
    code: `export const a = () => <div />;\n` },
  // The two false positives the old line-regex guard produced. Both must pass.
  { name: "lms-core allows a parameter named document", file: "src/lms-core/__guard_probe.ts", expect: null,
    code: `export function a(document: { id: string }) { return document.id; }\n` },
  { name: "lms-core allows a comment naming window", file: "src/lms-core/__guard_probe.ts", expect: null,
    code: `// This module must never reach for window.localStorage.\nexport const a = 1;\n` },

  // --- public route files compose, they do not draw -------------------------
  { name: "route file rejects a CSS import", file: "src/app/(platform)/__guard_probe/page.tsx", expect: "must not import CSS",
    code: `import "./probe.css";\nexport default function P() { return null; }\n` },
  { name: "route file rejects PlatformContentStyles", file: "src/app/(platform)/__guard_probe/page.tsx", expect: "PlatformContentStyles directly",
    code: `import s from "@/components/platform/PlatformContentStyles";\nexport default function P() { return String(s); }\n` },
  { name: "route file rejects a structural tag", file: "src/app/(platform)/__guard_probe/page.tsx", expect: "structural layout tags",
    code: `export default function P() { return <section>x</section>; }\n` },
  { name: "route file allows a plain element", file: "src/app/(platform)/__guard_probe/page.tsx", expect: null,
    code: `export default function P() { return <div>x</div>; }\n` },

  // --- the admin is grey ----------------------------------------------------
  { name: "admin rejects a colour utility", file: "src/app/(platform)/admin/__guard_probe.tsx", expect: "admin is grey",
    code: `export const a = <div className="bg-blue-500" />;\n` },
  { name: "admin rejects a gradient", file: "src/app/(platform)/admin/__guard_probe.tsx", expect: "admin is grey",
    code: `export const a = <div className="bg-gradient-to-r" />;\n` },
  { name: "admin rejects a colour inside a template literal", file: "src/app/(platform)/admin/__guard_probe.tsx", expect: "admin is grey",
    code: "export const a = (on: boolean) => <div className={`p-2 ${on ? \"text-red-600\" : \"\"}`} />;\n" },
  { name: "admin allows a grey utility", file: "src/app/(platform)/admin/__guard_probe.tsx", expect: null,
    code: `export const a = <div className="px-3 py-2 text-sm" />;\n` },

  // --- the layers -----------------------------------------------------------
  { name: "lib rejects an import from components", file: "src/lib/__guard_probe.ts", expect: "lib must not import",
    code: `import { PlatformStyles } from "@/components/platform/PlatformStyles";\nexport const a = PlatformStyles;\n` },
  { name: "components reject an import from app", file: "src/components/__guard_probe.ts", expect: "components must not import",
...
for (const { file, structuralRule } of exemptions) {
  ...
  if (applied === structuralRule) {
    ...
    failed += 1;
    console.error(`FAIL structural-tag rule should be ${structuralRule ? "on" : "off"} for ${file}, it is ${applied ? "on" : "off"}`);
  }
}
