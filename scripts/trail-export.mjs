...
for (const e of picked) {
  ...
  out.push({
    ...
    course: { slug: course?.slug, title: course?.title, version: course?.version, schedule: course?.schedule ?? null },
...
for (const t of out) {
  ...
  console.log(
    `${t.handle}  ${t.course.slug}  v${t.course.version}  lessons=${t.lessons.length}  ann=${t.annotations.length}  ${JSON.stringify(kinds)}`,
  );
