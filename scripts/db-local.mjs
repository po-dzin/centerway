...
function sync() {
  ...
  /* 000 — WHAT THE DUMP ALREADY CONTAINS (2026-09-13). The shape above is
     production after every migration it has applied, so replaying those files
     over it fails on the first policy or constraint that already exists — and
     `reset` used to replay all of them. The journal of what production applied
     lives in `supabase_migrations.schema_migrations` (docs lag it; see the
     migration-status note), so it is read at the same moment as the shape and
     written beside it. `reset` then applies only what is missing from it. */
  const applied = query(url, "select version from supabase_migrations.schema_migrations order by version");
  fs.writeFileSync(
    path.join(localDir, "000_applied_migrations.txt"),
    "# Migration versions production had applied when 010_schema.sql was dumped.\n" +
      "# Generated by scripts/db-local.mjs — do not hand-edit.\n" +
      applied +
      "\n",
  );
  console.log(`  000_applied_migrations.txt ${applied.split("\n").filter(Boolean).length} versions`);
...
function reset() {
  assertLocal();
  const allMigrations = fs.existsSync(path.join(rootDir, "supabase", "migrations"))
    ? fs
        .readdirSync(path.join(rootDir, "supabase", "migrations"))
        .filter((name) => name.endsWith(".sql"))
        .sort()
    : [];
  const appliedFile = path.join(localDir, "000_applied_migrations.txt");
  const applied = fs.existsSync(appliedFile)
    ...
    : null;
  /* A dump taken before the journal was recorded has no list. Then every file
     is a candidate, and each one runs in its own transaction below, so a file
     the dump already contains fails whole, changes nothing, and is reported —
     instead of aborting the reset halfway, as it did. */
  const staged = applied ? allMigrations.filter((name) => !applied.has(name.split("_")[0])) : allMigrations;
  ...
  if (!applied) {
    console.log(
      "  no 000_applied_migrations.txt — trying every migration; re-run `db:local:sync` to record the journal",
    );
  }
  const refused = [];
  for (const name of staged) {
    try {
      psql(LOCAL_DB_URL, ["--single-transaction", "-f", path.join(rootDir, "supabase", "migrations", name)], {
...
function env(off) {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "# The local stack has no Google provider; the sign-in door hides the button.",
