...
const { data: enr, error: e1 } = await c.from("lms_enrollments").select("id,course_id,created_at").limit(500);
if (e1) throw new Error(e1.message);

const { data: ann, error: e2 } = await c
  .from("lms_annotations")
  .select("id,enrollment_id,kind,lesson_id,note,created_at")
  .limit(5000);
if (e2) throw new Error(e2.message);

const { data: ev, error: e3 } = await c
  .from("lms_progress_events")
  .select("enrollment_id,type,lesson_id,occurred_at")
  .limit(20000);
