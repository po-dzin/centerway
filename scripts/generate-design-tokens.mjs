...
/* THE SECOND HALF OF THE DELIVERY LAYER (2026-09-10): the same `--ds-*` names,
   re-pointed for a fine pointer. Emitted inside
   `@media (hover: hover) and (pointer: fine)` rather than at a width, because
   what differs between a phone and a desktop here is the INPUT and not the
   canvas — a finger needs a thumb-sized target, a mouse does not, and a
   touchscreen laptop at 1440px needs the finger's numbers. Same query the
   hover rules and the `fill` fit take. */
...
/* The motion contract (2026-09-09): four durations and three curves, travelling
   for exactly the reason the radius and spacing scales did. The network was
   found running 180ms in 22 places, 160ms in 6 and 220ms in 6 — the platform's
   own scale, retyped — plus its ease-out written as `cubic-bezier(.2,.8,.2,1)`
   fourteen times and `cubic-bezier(.2, .8, .2, 1)` four more: one decision,
   typed two ways, because the name for it did not exist yet.

   --cw-ease-network IS that curve, kept as its own name rather than folded into
   --cw-ease-state. The two are genuinely different decisions — the landings
   decelerate harder than the platform does — and a token layer that quietly
   retimes eighteen landing sites on the day it is introduced is a visual change
   wearing a refactor's clothes. Whether the network keeps its own ease is a
   question for the day someone argues it; today the name just makes the
   existing answer sayable. See docs/design-system.md → "Movement". */
const NETWORK_MOTION_TOKENS = [
  "--cw-motion-tap",
  "--cw-motion-state",
  "--cw-motion-surface",
  "--cw-motion-enter",
...
function pickMotionTokens(...maps) {
...
async function main() {
  ...
  const pointerFineDecls = toDecls(tokens.delivery?.dsAliasPointerFine ?? {}, "    ");
