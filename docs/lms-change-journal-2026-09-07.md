...
Задача называлась «версии уровня урока», а снимок остался курсовым. Это не
компромисс, а решение, и вот его причина: **урок не самостоятелен**. Перенос
блока между уроками, перестановка модулей, готовность, считающаяся по всему
курсу, — ни у чего из этого нет корректного представления «на один урок», а две
независимые истории начали бы расходиться в первый же такой день. Что до
восстановления, оно тоже курсовое: откатить один урок, оставив остальные, значит
получить состояние, которого никогда не существовало.

Поэтому «версии этого урока» — **фильтр** по истории курса, а не вторая история.
`GET .../revisions?lesson=<id>` идёт по записям от старой к новой, сравнивает
соседние тем же `diffCourses` и оставляет только те, где изменился именно этот
урок. Без такого отбора история урока повторяла бы историю курса, где правка
была где угодно ещё, — то есть отвечала бы не на заданный вопрос.

Цена названа честно: в отличие от списка версий курса, этот путь ГРУЗИТ
содержимое ревизий — понять, менялся ли урок, нельзя не заглянув внутрь. Отсюда
предел в последние 60 записей и открытие по кнопке, а не при каждом рендере.

### Где это стоит

Часы (`clock`) в органах шелла — и на уровне курса, и теперь в редакторе урока.
Вопрос «что происходило с этим уроком» задают, глядя на урок, а не на список
курсов. В режиме урока заголовок записи говорит, что случилось именно с ним
(«3 блоків змінено», «перейменовано»), а не название контрольной точки: оно
отвечает не на то, с чем сюда пришли. Правка «межі» помечается и здесь.

Кнопка «сохранить контрольную версию» в этом режиме скрыта: сохраняется курс
целиком, и предлагать «версию урока» значило бы обещать то, чего нет.
