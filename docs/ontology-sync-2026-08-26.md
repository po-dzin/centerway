...
| 16 | Локаль | канон молчит про UI-локаль | метааудит §6.4 (ред. 2026-08-27): `uk/en` везде | **закрыто 2026-08-27**: один код `uk` во всех трёх местах, `ru` снят, `ua` схлопнут в `uk` | ✓ |
...
| Локаль | **Locale = `uk | en`**, один код на всё (решение 2026-08-27) | `Lang = ru|en`, `Locale = ua|en` | схлопнуто: `ua` → `uk`, `ru` снят с поверхностей |
