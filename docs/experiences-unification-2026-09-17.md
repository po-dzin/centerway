...
3. **Один оффер.** ✅ `20260924020000` и `20260924030000`: одна таблица цен, наборы,
   ...
   ✅ 26.09 (`feat/money-boundary`): любой код оффера — чекаут, возврат, заявка,
   ссылка доступа, Meta, отчёты, корпус агента, цены в админке — идёт через
   `describeOffer` → `offer_aliases`; заказ пишется под собственным кодом оффера.
   `normalizeProduct` и `PRODUCTS` удалены целиком.
4. **Шлюз как граница и доли.** ✅ 26.09: `src/lib/payments/gateway` (интерфейс +
   WayForPay), `author_payout_accounts`, `order_shares` с триггером на оплату.
   Второй шлюз (LiqPay) ждёт решения из шага 0; наборы из вещей разных авторов и
   доли амбассадоров не покрыты.
