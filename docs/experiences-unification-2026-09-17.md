...
### 2.3 `experience_offers` + `experience_offer_items`
...
1. **Поток и ссылка** — на текущей схеме. ✅ якорь потока, `dripAnchor`, `?ref` и
   UTM через cookie до enrollment и заказа, дата и метка в ручной выдаче.
   Осталось: счётчик амбассадора в кабинете, состав потока в админке,
   `orders.fulfilment_status`.
2. **Реестр и имена.** ✅ миграция, триггеры, backfill, `src/lib/experiences`,
   билдер обходит занятые имена. Осталось: коды в `offer_aliases` (после шага 3).
3. **Один оффер.**
