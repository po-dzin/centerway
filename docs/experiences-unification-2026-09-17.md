...
### 2.1 `experiences` — узкий реестр  ✅ `20260919000000`
...
### 2.3 `experience_offers` + `experience_offer_items`  ✅ `20260920000000`
...
`20260919010000`: `cohort_starts_on date` (общий день 1 потока, календарная дата в
...
`orders`: `ref` ✅ (`20260919010000`); `experience_id` и `offer_id` — вместе с офферами.
Доли — строками:
...
1. **Поток и ссылка** — на текущей схеме. ✅ якорь потока, `dripAnchor`, `?ref` и
   ...
   `pending`. Осталось: счётчик амбассадора в кабинете; статус исполнения в
   интерфейсе заказов и заявок.
2. **Реестр и имена.** ✅ целиком: миграция, триггеры, backfill, `src/lib/experiences`,
   билдер обходит занятые имена, коды в `offer_aliases`.
3. **Один оффер.** ✅ `20260920000000`: `experience_offers`, `experience_offer_items`,
   `offer_aliases`, зеркало старых таблиц триггерами, проза счёта дословно из
   `PRODUCTS`, заказ узнаёт вещь и оффер триггером по `product_code`; два публичных
   читателя цены переключены. Осталось: резолв кода через `offer_aliases` вместо
   `normalizeProduct`; срок доступа из новой таблицы (`server.ts`, `access/payments.ts`,
   `shelfHealth`); `resolveEntitlement` через `offer_items`; каталог пишет в новую
   таблицу (после этого зеркало удаляется); несколько офферов на странице вещи.
4. **Шлюз как граница и доли.** Второй шлюз ждёт решения из шага 0.
5. **Автор там, где он зашит** (три места) + sitemap/llms.txt из базы.
6. **Уборка.**
...
У каждого `thanks.html`, `pay-failed.html`, `public-offer.html` (WFP хранит return URL, `products.ts:19-21`); `index2.html` не используются. Инъекция цен: `priceSync.ts`, `landingPrices.ts`, `prepareLandingHtml.ts`; shared JS `checkout, lead-form, landing-pixel, funnel-network, proof`.
