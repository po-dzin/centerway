...
**The scroller seals its own stacking context (2026-09-05), and here is the bug that made it a rule.** A course tile's `КУРС` kind badge (`.programTileKind`, `z-index: 1`) painted *over* the offer rail's arrow. Neither `.viewport` nor `.programTile` creates a stacking context, so the badge's `z-index` did not stop at its own card — it escaped all the way up to `.stage`, the nearest ancestor that actually is one. There it competed directly with `.controls`, `.stage`'s other child, *also* `z-index: 1`. On that tie the later element in the DOM wins, and `.viewport` is written after `.controls` — so the badge won, on every rail, the moment any card used `z-index: 1` for anything.

The fix is not a higher number on `.controls`: that wins today's tie and loses the next one, the first time a card nests a `z-index: 2` for its own overlay. It is `isolation: isolate` on the scroller itself — `.viewport` on the platform, `.car-track` and `.testimonial-carousel__track` on the network — so a card's `z-index` can only ever stack against *its own rail*, never against the arrow standing outside it. The arrow is the one thing not inside that box, so it is unconditionally above everything that is. Any future card, on any rail, can declare whatever `z-index` it needs for its own layering without ever becoming this bug again.
...
Each now carries both blocks itself, and **the value is not shared between them**, deliberately. Chrome has one fallback number (94%) because chrome has one backdrop: a bounded band. These four sit on photographs, where the backdrop is whatever the author uploaded, so what the blur was buying differs per element and so does what replaces it — the kind chip closes to a fully opaque accent plate because its remaining 12% was texture the blur paid for, the category chips deepen from 42% to 68% ink because the blur was their separation from the cover, the step chip takes the material's own `--cw-mat-surface` because it is plain canvas glass that simply forgot to say so, and `onMedia` takes chrome's 94% because a control over a picture that goes fully solid stops resting on the image and starts reading as a hole cut in it.

The general rule this leaves: **a hand-written `backdrop-filter` owns its own degradation.** Either declare `[data-cw-material]` and inherit the contract, or write both blocks at the rule. `guard:contrast` pins each fallback opacity alongside the resting state it replaces, so a number chosen here cannot be thinned later without the gate saying so.
...
One number for the room the hidden bar gave back, on every shell that hides one: `max(--cw-page-gutter, safe-area-top) + --ds-touch-target-min + air` — 84px on a phone with `--cw-space-md` of air. Measured identical in `PlatformResponsive`, `Builder.module.css`, the panel's Tailwind `pt-[5.25rem]`, the course tab strip's sticky offset, and the reader's three (`Lms.module.css`, `ZenPreview.module.css` — those had been computing the first term as `--cw-space-sm`, 8px short of the row they were reserving for, and were corrected on 2026-09-07). If one moves they all move.
...
**One way to wait (2026-09-06).** The mark gaining density in turn — `LogoMark animate="wait"` — is the waiting state, and it has said so in its own source since it was written («the spinner replacement»). Three places had not heard: the admin panel span a Tailwind ring, the diagnostic span a CSS one, and a retry button span a 14px one. A rotating circle says «something is happening» without saying what, next to copy that says exactly what — and it belongs to no design system in particular. The skeleton row is not a second answer but a different question: use it where the shape of what is coming is already known (a table about to have rows), and the mark where it is not. Below 20px neither applies — a control in flight is disabled and says so in words, with the ellipsis every other control here uses («Зберігаємо…»).

**One scale for layers (2026-09-06).** `--ds-z-base` 1 · `--ds-z-sticky` 999 · `--ds-z-modal` 1200, and everything else states itself as `calc()` off one of them: the sheet at `sticky + 2`, its shield at `+1`, the chrome raised over its own sheet at `+3`. Two places had opted out and both were wrong by more than a number: the auth dialogue painted its shield with Tailwind utilities at `z-50` — a rung from a scale this product does not run, two orders below the topbar it had to cover — and the floating chrome used a bare `4`, which put it under a sticky strip at 998. The islands are the bar one viewport down, so they take the bar's own rung.

**Ink marks text; a box marks itself (2026-09-06).** The hand's stroke and ring exist because a list of names is not a list of buttons: a row with no ground, no contour and no weight of its own has nothing to change when it becomes current, so it borrows the pen. A control that already HAS a box — a plated island, a segmented control, a filled button — states the same thing with what it is: its ground, its contour, its weight. Drawing both is one state said twice, and it reads as two systems arguing rather than as one.

The boundary is the box, not the component. The account entry is the case that proves it: in the bar it is transparent, so the ring is its only mark and stays; floating, the same control wears the island's plate and then sits inside the sheet it opened, so the ring never paints there. Nothing else moved — nav labels, menu rows, the trail, the outline, the course tabs are all bare text and keep the ink they have had.

**Soft rect, not circles (2026-09-06).** The islands shipped as pills — a floating control is habitually a circle — which is precisely the borrowed default this system replaced everywhere else: on the one radius scale, `pill` is for things that are genuinely round, and a 48px square control is not one. Every island now takes `--cw-radius-md`, the control step, and the sweep went through the rest of the platform with it: `--platform-utility-control-radius` (the account entry, the bar's menu button, the panel close), the bar's nav links, the three theme options, the diagnostic step chip, and **every portrait** — the cabinet's 3.75rem face, the byline's 5.5rem and the author page's 5–8rem take `lg`, and the crop editor's 10rem preview takes `xl`, because a crop editor that previews a corner the page does not draw is the exact failure it exists to prevent.

What stays round is what the scale always said: things that are **genuinely round** — the progress bars and rails, the carousel dots, the wheel and notice dots, the spinner, the drawer handle. Text chips and badges keep their pill for now; they are a separate decision from circles.

One consequence worth writing down: the hand-drawn ink ring is a CIRCLE, and a circle that cleared a 3rem disc does not clear a 3rem square — the corners sit on its diagonal, 4.24rem. Both ring ratios were re-measured against the diagonal (1.2 in the bar, 1.5 in the islands) or the stroke would cross the face again, in a new place.
...
### The chrome, corrected in five places (2026-09-07)

**The islands stand on the page's own gutter.** They were inset by `--cw-page-gutter` and spent an afternoon at `--cw-space-sm`, one step tighter, on the argument that chrome standing on the article's left edge reads as the article's first row. Fixing the thing underneath removed the argument: the workshop and the panel were on 16 and 12px while the platform was on 20, and once all four surfaces took the token there was one margin in the product to align to — a floating object one step inside a margin nobody else uses is not «clearly chrome», it is a fourth number. What says the pair floats is its plate, its shadow and the way it steps aside on scroll. That is what those are for.

**Both islands are one plate with one figure in it.** The signed-in control used to be a photograph filling its whole 3rem box, opposite a 2rem mark inside a plate — two corners that still did not match, they only stopped mismatching in the same direction. The portrait is the mark's own 2rem now, centred in the same plate. It is the one **round** thing in the chrome, and that is not an exception to «soft rect, never pill»: the rule is about the box a control occupies, and the box here is the island. A square crop inside a square plate is the same corner drawn twice, 8px apart.

**The island sheet's rows take a panel's inset, not the mark's.** First pass: the islands are positioned by the fixed row, which is the sheet's *border* box, so the panel's own inline padding pushed every row further in than the pair resting on top of them — the mark's optical inset (half of 3rem − 2rem) landed 0.65rem short of the first letter, and the rows were pulled onto that inset so the two would share a left edge. They did, and it was wrong: 0.5rem is not an inset, it is the air *inside* an object. The mark is centred in a plate and the plate's edge is what stands at the sheet's edge; text on a panel has no plate, so aligning it to a figure's internal air puts the words 8px from a glass edge and the column reads as though it had lost its padding. What has to line up is the two EDGES, and they already do — the plate and the sheet both stand on `--cw-page-gutter`. Inside, the rows take `--cw-space-md`; the panel still gives its own inline padding up so that one rule owns the number.

**The island sheet shields the whole screen.** A drawer shields the page and never the bar it hangs off — true for the two anchored forms, and false for this one, which *begins* at the row and takes the pair inside itself. Starting the shield at the sheet's top edge left an unblurred band between the status bar and the panel. The island form starts at 0.

**The theme control's recess comes back, measured by the word.** Removing it (2026-09-06) was half right: the sheet does group the row, but three loose glyphs at the end of a line still read as three items. What was wrong was the weight — a 3rem pill wrapping three 3rem hit boxes made a setting the heaviest object in a menu of words. The rail is now the height of the line of text it shares the row with (`--ds-type-body-size` × its line height), at 6% of the sheet's own ink, drawn as a pseudo-element so it cannot move the seats. The hit boxes are untouched: a finger still lands on 3rem.

**And the preview's way back gave its word up (2026-09-07).** It was the one leading control in the product that carried a label, on the argument that an author standing inside the learner's skin reads a bare arrow as «back to the shelf». The loading screen settled it: an empty sheet with one captioned pill on it, before the course the author asked to see had arrived — the chrome as the subject of the page. It is `.organ` like every other island now, with «До редагування» in the `aria-label` and the tooltip.

### One gutter, and it is `--cw-page-gutter` (2026-09-07)

Three numbers were running down the side of a phone: the platform and the library on `--cw-page-gutter` (`clamp(20px, 5vw, 40px)` — 20px at 375), the workshop on a bare `2rem` box inset (16px a side), and the Control Panel on Tailwind `px-3` / `sm:px-4` (12, then 16). Nothing chose them; each surface answered the question locally and the same course card sat at three different distances from the edge depending on which app was drawing it.

The token wins, because it is the only one of the three that is a decision rather than a habit: it is what every public page, the shelf and the library already measure against, and it is the number the chrome's own inset is now deliberately *different* from (`--cw-space-sm`, so the floating pair does not stand on the article's left edge). The workshop's document width and the panel's scroll pane both read it now; the desktop rules that widen either of them are unchanged — they extend this formula rather than replacing it.

### Ink follows the theme (2026-09-07)

**Black on the day gamma, gold on the night one, and nothing else moves it.** Two things used to: the theme, and what the header measured behind itself. So one screen marked «you are here» in two colours — gold in a bar over a hero, near-black in the page under it, and the reverse again on a portalled menu carrying the other stamp. A reader cannot learn a mark that changes colour by backdrop.

One exception, and it is the rule restated rather than broken: a bar **wearing** the night material (`[data-cw-header-tone="dark"]`) keeps the gold, because its plate is graphite and its cream text sits on it — near-black there is not a quieter mark but an invisible one, and the current nav row, which takes the marker as its own colour, disappeared outright. Ink on a pale ground, gold on a dark one; that bar is a dark ground by measurement. What does **not** come back is the other half: `[data-cw-header-tone="light"]` no longer re-declares the ink, so a light stamp can never force near-black onto the night theme.

### One scrollbar (2026-09-07)

Three answers, in `globals.css`, and every scroller in the product takes one: `cw-scroll` (a viewport with its own bar), `cw-scroll-rail` (4px, for a strip where a 10px bar is the tallest thing in the row), `cw-scroll-none` (no bar — for a **snap rail**, which reports its position with its cut edge and its dots). The thumb is ink — a wash of the page's own text via `--cw-mat-scroll-*` — and the track is nothing.

What that replaced: the tab strip's own 4px copy, the builder writing the same 22% ink out by hand for nine scrollers, seven modules each spelling the two lines that hide a rail, and three admin tables asking for a `no-scrollbar` class **that was never defined** — the bar they meant to hide had been on screen the whole time. `custom-scrollbar` and `cw-tabbar` survive as aliases on the one rule, not as second recipes. CSS modules reach it with `composes: cw-scroll-none from global`, which is why these are classes and not attributes.
