...
**One left edge in the island sheet.** The islands are positioned by the fixed row, which is the sheet's *border* box, so the panel's own inline padding pushed every row further in than the pair resting on top of them — the mark's optical inset (half of 3rem − 2rem) landed 0.65rem short of the first letter. The panel gives its inline padding up and the rows carry it instead, at exactly that inset.
