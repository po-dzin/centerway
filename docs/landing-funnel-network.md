...
- Cross-node navigation is carried by **all five nodes** via one shared component: `shared/css/network-nav.css` + `shared/js/network-nav.js` (burger below 760px, topbar at 760px+, `--cw-nav-*` component tokens with per-landing palette mapping; human-readable Ukrainian labels: Шлях 21 / Розвантажувальний день / Тест доші / Фітозбори / Консультація). It is absent from the first hero and has no layout footprint; after the hero it appears as a fixed top navigation only while scrolling upward, then hides again while scrolling downward. The `way21 ↔ reset-day` pair is additionally linked by content bridges both ways.
