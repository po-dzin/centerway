...
- Shared network runtime: `src/landing-static/shared/js/funnel-network.js` (reveal, sticky CTA, smooth anchors, cross-node nav block). Its stylesheet counterpart `shared/css/funnel-network.css` was deleted on 2026-09-11: nothing had loaded it since the landings moved onto `shared/css/landing.css`, and its `.cw-nav` block collided by name with the live short/IREM header.
...
- **One DS for all five — a single token contract.** Per the ReOS canon (`Дизайн-токени.md`), the design system *is* the token contract, not any one component stylesheet. `shared/css/network-tokens.css` is that contract for the whole network: it defines the semantic `--cw-net-*` layer (mineral-green core defaults + one per-landing skin block each) and is linked first on all five landings. Both component dialects consume it:
  ...
  - dosha/consult/herbs read `--cw-net-*` straight from this contract — the old `funnel-network.css` dialect (`--cw-accent`, `--cw-gold`, …) is gone with the file, and those three carry no `page.css` at all.
