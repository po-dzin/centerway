...
function assertLandingEntryAndView() {
  const shortIndex = readLocal("public/lps/short/index.html");
  const iremIndex = readLocal("public/lps/irem/index.html");

  assert.match(shortIndex, /class="openModal"/, "short landing must have CTA openModal");
  assert.match(shortIndex, /id="precheckoutModal"/, "short must have precheckout modal");
  assert.match(shortIndex, /id="precheckoutForm"/, "short must have precheckout form");
  assert.match(shortIndex, /id="precheckoutName"/, "short must have name input");
  assert.match(shortIndex, /id="precheckoutEmail"/, "short must have email input");
  assert.match(shortIndex, /id="precheckoutPhone"/, "short must have phone input");
  ...
  assert.match(iremIndex, /id="precheckoutName"/, "irem must have name input");
...
function assertCheckoutClientFlow() {
  const shortJs = readLocal("public/lps/short/js/common.js");
  const iremJs = readLocal("public/lps/irem/js/common.js");

  assert.match(shortJs, /CHECKOUT_ENDPOINT = API_BASE \+ "\/api\/checkout\/start"/, "short must call /api/checkout/start");
  assert.match(shortJs, /trackInitialCheckout\(\)/, "short must track initial checkout when opening precheckout form");
  assert.match(shortJs, /name:/, "short request payload must include name");
  assert.match(shortJs, /email:/, "short request payload must include email");
  assert.match(shortJs, /phone:/, "short request payload must include phone");
  assert.match(shortJs, /fbq\("track", "Lead"/, "short submit must fire Lead event");
  ...
  assert.match(iremJs, /name:/, "irem request payload must include name");
  ...
  assert.match(iremJs, /fbq\("track", "Lead"/, "irem submit must fire Lead event");
...
async function assertCheckoutAndLeadsPersistence() {
  ...
  const lead = buildLeadRecord(body, "irem", "irem_20260213_deadbeef");
...
function assertReturnAndPostPaymentPages() {
  ...
  const shortThanks = readLocal("public/lps/short/thanks.html");
  const iremThanks = readLocal("public/lps/irem/thanks.html");
  const shortFailed = readLocal("public/lps/short/pay-failed.html");
  const iremFailed = readLocal("public/lps/irem/pay-failed.html");
...
function assertNoCrossDomainLeak() {
  const shortOffer = readLocal("public/lps/short/public-offer.html");
  const iremOffer = readLocal("public/lps/irem/public-offer.html");
